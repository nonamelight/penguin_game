...
class DayBackground extends ParallaxComponent<ClubPenguinGame> with HasGameRef<ClubPenguinGame> {
  DayBackground();
  ...
  Future<void>? onLoad() async {
    ...
    final dayImages = [
      ...
      gameRef.loadParallaxImage('background/screen1/7.png', fill: LayerFill.none),
    ];
    final dayLayers = dayImages.map((image) async => ParallaxLayer(await image, velocityMultiplier: Vector2(1, 0)));
...
class NightBackground extends ParallaxComponent<ClubPenguinGame> with HasGameRef<ClubPenguinGame> {
  NightBackground();
